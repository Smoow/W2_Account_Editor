...
namespace EditorAccounts
{
    public partial class Form4 : Form
    {
        ...
        {
            W2___Pesquisar_Item pesquisar = new W2___Pesquisar_Item(this);
            pesquisar.Show();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (DataGridViewRow row in Form1.dataGridView1.SelectedRows)
                {
                    ...
                    if (File.Exists("./imagens/" + txtItemIndex.Text + ".png"))
                    {
                        row.Cells[1].Value = Image.FromFile("./imagens/" + txtItemIndex.Text + ".png");
                    }

                    row.Cells[2].Value = txtItemIndex.Text;
                    row.Cells[3].Value = txtEF1.Text;
                    row.Cells[4].Value = txtEFv1.Text;
                    row.Cells[5].Value = txtEF2.Text;
                    row.Cells[6].Value = txtEFv2.Text;
                    row.Cells[7].Value = txtEF3.Text;
                    row.Cells[8].Value = txtEFv3.Text;

                    int index = Convert.ToInt32(row.Cells[0].Value);

                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sIndex = Convert.ToInt16(txtItemIndex.Text);
                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sEffect[0].cEfeito = Convert.ToByte(txtEF1.Text);
                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sEffect[0].cValue = Convert.ToByte(txtEFv1.Text);
                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sEffect[1].cEfeito = Convert.ToByte(txtEF2.Text);
                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sEffect[1].cValue = Convert.ToByte(txtEFv2.Text);
                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sEffect[2].cEfeito = Convert.ToByte(txtEF3.Text);
                    External.g_pAcccount.Char[External.CurrentChar].Carry[index].sEffect[2].cValue = Convert.ToByte(txtEFv3.Text);
